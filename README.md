...
| `RAINBOW_URLS` | No | Comma-separated list of URLs for services implementing `com.atproto.sync.listReposByCollection` (e.g. Rainbow, Lightrail) used for backfill discovery at startup. Falls back to the next URL on error. Defaults to `https://bsky.network`. |
